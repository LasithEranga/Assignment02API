...
public class ToDo {
    ...
    private static final String SEARCH_TODOS = "SELECT * FROM todos WHERE todo_id = ?";
    private static final String UPDATE_TODO = "UPDATE todos SET title= ? , description= ? , date = ? , time = ? , state= ? , category = ? WHERE todo_id = ?";
    private static final String MARK_AS_COMPLETE = "UPDATE todos SET state = 'completed' WHERE todo_id = ?";
    ...
    public static String update(ToDo todo, int id) {
        try (Connection con = Database.getConnection()) {
            ...
            System.out.println(updateToDo.executeUpdate());
    ...
    public static String markAsComplete(int id) {
        try (Connection con = Database.getConnection()) {
            PreparedStatement updateToDo = con.prepareStatement(MARK_AS_COMPLETE);
            updateToDo.setInt(1, id);
            updateToDo.executeUpdate();
            String response = "{'message':'Todo Markded as completed!'}";
            JsonElement jsonElement = JsonParser.parseString(response);
            JsonObject jsonObject = jsonElement.getAsJsonObject();
            return new Gson().toJson(jsonObject);
        } catch (Exception e) {
